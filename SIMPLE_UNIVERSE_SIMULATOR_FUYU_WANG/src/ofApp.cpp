...
void ofApp::update() {
	...
	for (int i = 0; i < emitter.triangles.size(); ++i) {
		emitter.triangles[i].mass = particleMass;
	}
	
	for (int i = 0; i < emitter.hexagons.size(); ++i) {
		emitter.hexagons[i].mass = particleMass;
	}
...
void ofApp::draw() {
	...
	// Info
	ofSetColor(255);
	ofDrawBitmapString("SPACE=reset | R=reset cam | F=fullscreen | H=GUI | WASD=move attractor | Mouse=rotate camera", 10, ofGetHeight() - 10);
}
...
void ofApp::keyPressed(int key) {
	...
	if (key == 'a' || key == 'A') {
		xMove = std::max((float)xMove - step, -(float)ofGetWidth() / 2);
	}
	if (key == 'd' || key == 'D') {
		xMove = std::min((float)xMove + step, (float)ofGetWidth() / 2);
	}
	if (key == 'w' || key == 'W') {
		yMove = std::max((float)yMove + step, -(float)ofGetHeight() / 2);
	}
	if (key == 's' || key == 'S') {
		yMove = std::min((float)yMove - step, (float)ofGetHeight() / 2);
	}
}
